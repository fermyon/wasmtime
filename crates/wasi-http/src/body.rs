...
impl Body for BodyWithTimeout {
    ...
    fn poll_frame(
        ...
        match result {
            ...
            Poll::Ready(Some(Err(e))) => Poll::Ready(Some(Err(match e.downcast() {
                Ok(e) => e,
                Err(e) => types::Error::ProtocolError(format!("{e:?}")),
            }))),
        }
    }
}
...
pub struct HostIncomingBody {
    ...
    worker: Option<Arc<AbortOnDropJoinHandle<Result<()>>>>,
}
...
impl HostIncomingBody {
    ...
    pub fn retain_worker(&mut self, worker: &Arc<AbortOnDropJoinHandle<Result<()>>>) {
    ...
    async fn ready(&mut self) {
        ...
        if let IncomingBodyState::InBodyStream(rx) = &mut body.body {
            match rx.await {
                ...
                // result.
                Ok(StreamEnd::Trailers(Some(t))) => *self = Self::Done(Ok(t)),
                Ok(StreamEnd::Trailers(None)) => *self = Self::Done(Ok(Default::default())),
